# Backend/API Integrator HEARTBEAT.md Addendum

This repo-tracked addendum mirrors Backend/API heartbeat handling rules for
...
- If system-provided continuity names a Workboard card, Task, PR, room, or
  wake-guard for Backend/API-owned contract, schema, mock, fixture, reviewer,
  service-evidence, `03-contract-api`, or PR handoff work, treat that named item
  as current active work even when the local Active Items list is empty.
...
- Do not reply `HEARTBEAT_OK` while current system-provided Backend/API
  continuity work is present. Continue, resolve, record a blocker, or preserve a
  wake/follow-up condition.
- If `/workspace/HEARTBEAT.md` is empty or comment-only and no current
  Backend/API continuity is named, reply `HEARTBEAT_OK` rather than inferring
  old work.
