...
Room Text Delivery Harness source lives under
`mobile-app-dev-team/runtime-sources/harnesses/room-text-delivery/`. Its
operational copy is generated by `openclaw-pod-skills-sync` at
`/workspace/harness/room-text-delivery/`. Deployment verification should check
that the sync report has `status=completed`, `blockers=[]`, and
`room_text_delivery.cmp=true`, and that runtime `MANIFEST.json` and
`DO_NOT_EDIT.md` are present. Do not edit the generated runtime copy directly;
fix the repo source and rerun sync.
...
### Room Text Delivery Harness Evidence

Use the Room Text Delivery Harness only when the accepted task, reviewer, or
Product/Planning asks for visible Room Text delivery proof. The harness proves
visible Room Text delivery only; it does not prove Task, Workboard, PR, or work
completion, semantic sufficiency, reviewer acceptance, release readiness, or
production readiness.

Live report destination is the current instruction Room unless a later explicit
report destination overrides it. Do not copy actual operating Room ids into
reusable docs, fixtures, examples, or harness evidence templates; use synthetic
test ids such as `1001` and `1002` for durable examples.

Success proof should include the normalized result and validator output, matching
`intended_room_id` and `actual_room_id`, a present `message_id`, 2xx
`http_status`, `transport_ok=true`, and `visible_report_destination_bound=true`
for report-delivery proof. Failure proof should include the relevant expected
FAIL cases for the task, such as plain text without a normalized result,
intended/actual Room mismatch, or access-boundary HTTP 403.

Reviewer-accessible evidence must be a PR comment or body, committed artifact,
Task attachment or comment, or equivalent shared record. Local
`/workspace/state/...` paths alone are internal tracking and are not sufficient
cross-pod evidence.
