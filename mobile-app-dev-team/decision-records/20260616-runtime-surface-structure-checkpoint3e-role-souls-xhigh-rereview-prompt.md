# Checkpoint 3-E Role SOUL XHigh Rereview Prompt

You are `wm-implementation-reviewer`.

Review mode: final checkpoint rereview for Checkpoint 3-E only.
...
Initial Checkpoint 3-E review result:

```text
.evidence/reviews/20260616-runtime-surface-structure-checkpoint3e-role-souls-xhigh-review.md
```

Initial verdict was `NO_GO` for:

- stale active `.claude` references to
  `mobile-app-dev-team/02-role-souls/`;
- incomplete stale-reference evidence because the scan omitted `.claude/`.

Fixes applied for rereview:

- `.claude/skills/best-practices/rules/_role-index.md` now references
  `mobile-app-dev-team/runtime-sources/role-souls/`.
- `.claude/skills/best-practices/rules/test-infrastructure.md` now references
  `mobile-app-dev-team/runtime-sources/role-souls/qa-release-soul.md`.
- Command evidence now includes a corrected stale-reference scan covering
  `.claude/`, `.agents/`, `.codex/`, scripts, evals, docs, package metadata,
  GitHub workflow metadata, and active `mobile-app-dev-team` docs while
  excluding historical archive/evidence records.
...
Checkpoint under rereview:

```text
3-E: role-souls/ move to runtime-sources/role-souls/
```
...
- Active role SOUL references in source map, team README, role-contract
  reference docs, `.claude` skill-rule docs, pod-native runtime specs,
  repo-local skill references, runtime-surface reports, validators, and
  project-bootstrap smoke fixtures align with
  `runtime-sources/role-souls`.
...
2. Verify the two initial NO_GO findings are fixed.
3. Verify all six target role SOUL paths exist in the staged index and the old
   `mobile-app-dev-team/02-role-souls/` paths are absent from the staged index.
4. Verify this does not include the Checkpoint 3-F pod-native OpenClaw source
   root move or the Checkpoint 5 source-map filename move.
5. Verify active docs and runtime specs now use
   `mobile-app-dev-team/runtime-sources/role-souls/`.
6. Verify validators require the new target root and reject the old
   `02-role-souls` legacy root.
7. Verify remaining `02-role-souls` references outside `_archive/` and
   `.evidence/` are limited to the approved goal-plan mapping/risk note and
   validator legacy/stale checks.
8. Verify the recorded command evidence is sufficient:
   - RED `pnpm run validate:team-doc` failed before physical move.
   - `pnpm run validate:team-doc` passed after move and again after the
     `.claude` fix.
   - `pnpm run test:runtime` passed after move and again after the `.claude`
     fix.
   - `pnpm run test:local-harness` passed after move and again after the
     `.claude` fix.
...
9. Identify any finding that should block proceeding to Checkpoint 3-F.
