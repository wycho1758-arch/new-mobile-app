# Checkpoint 3-F Pod-Native Source Root XHigh Rereview Prompt

You are `wm-implementation-reviewer`.

Review mode: final checkpoint rereview for Checkpoint 3-F only.
...
Initial Checkpoint 3-F review result:

```text
.evidence/reviews/20260616-runtime-surface-structure-checkpoint3f-pod-native-xhigh-review.md
```

Initial verdict was `NO_GO` for one active stale reference:

- `CLAUDE.md:61` referenced
  `mobile-app-dev-team/09-pod-native-openclaw-skills/<slug>/`;
- the stale-reference evidence omitted root `CLAUDE.md`.
...
- Command evidence now includes a corrected stale-reference scan covering
  `CLAUDE.md` plus the prior active trees.
- `pnpm run validate:team-doc`, `pnpm run test:runtime`, and
  `pnpm run test:local-harness` were rerun after the fix and passed.
...
- Update active pod-native source references in root policy docs, root
  `CLAUDE.md`, project environment docs, repo operations docs, source map, team
  README, runtime surface/reference reports, durable work-unit docs, validators,
  `.claude` memory, and smoke fixtures.
...
2. Verify the initial `CLAUDE.md` stale-reference finding is fixed.
3. Verify all old pod-native source-root files are staged as moves to
   `runtime-sources/pod-native-openclaw-skills/`.
4. Verify the old `mobile-app-dev-team/09-pod-native-openclaw-skills/` root has
   no staged index entries.
5. Verify this does not include the Checkpoint 5 source-map filename move.
6. Verify `openclaw-pod-skills-sync` and `project-bootstrap` now default to the
   new source root, including smoke fixtures.
7. Verify active docs and runtime specs now use
   `mobile-app-dev-team/runtime-sources/pod-native-openclaw-skills/`.
8. Verify validators require the new target root and reject the old legacy root.
9. Verify remaining `09-pod-native-openclaw-skills` references outside
...
10. Verify the recorded command evidence is sufficient:
   - RED `pnpm run validate:team-doc` failed before physical move.
   - `pnpm run validate:team-doc` passed after move and after the `CLAUDE.md`
     fix.
   - `pnpm run test:runtime` passed after move and after the `CLAUDE.md` fix.
   - `pnpm run test:local-harness` passed after move and after the `CLAUDE.md`
     fix.
...
11. Identify any finding that should block proceeding to Checkpoint 4.
